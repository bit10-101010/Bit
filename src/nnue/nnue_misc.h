...
template<>
struct std::hash<Bit::Eval::NNUE::EvalFile> {
    std::size_t operator()(const Bit::Eval::NNUE::EvalFile& evalFile) const noexcept {
        std::size_t h = 0;
        Bit::hash_combine(h, evalFile.defaultName);
        Bit::hash_combine(h, evalFile.current);
        Bit::hash_combine(h, evalFile.netDescription);
        return h;
    }
};
