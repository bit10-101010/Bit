...
struct std::hash<Bit::Eval::NNUE::Network<ArchT, FeatureTransformerT>> {
    std::size_t operator()(
      const Bit::Eval::NNUE::Network<ArchT, FeatureTransformerT>& network) const noexcept {
        return network.get_content_hash();
    }
};

template<>
struct std::hash<Bit::Eval::NNUE::Networks> {
    std::size_t operator()(const Bit::Eval::NNUE::Networks& networks) const noexcept {
        std::size_t h = 0;
        Bit::hash_combine(h, networks.big);
        Bit::hash_combine(h, networks.small);
        return h;
    }
};
